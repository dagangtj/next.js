...
> **Warning:** By default, revalidation is **instance-local**. Calling `revalidateTag()` only invalidates the cache on the instance handling the request. In multi-instance or serverless environments, other instances may continue serving stale content. For distributed setups, see [Multi-Instance Considerations](/docs/app/guides/how-revalidation-works#multi-instance-considerations).
